...
import SegmentDonut from '../../../components/ui/SegmentDonut.jsx';
...
import FormLabel from '../../../components/forms/FormLabel.jsx';
...
import { stockTabs, getStockResource } from '../config/stockTabs.js';
...
export default function StockHubPage() {
    ...
    const updateField = (key, value) => {
        if (key === 'building_name' && !editingId) {
            const selected = (data?.buildings || []).find((b) => String(b.id) === String(value));
            const bName = selected?.name || '';
            const match = bName.match(/B-(\d+)/);
            const num = match ? parseInt(match[1], 10) : 0;
            const autoType = num >= 1 && num <= 3 ? 'Growers' : num >= 4 ? 'Layers' : '';
    ...
    const submit = async (event) => {
        ...
        try {
            ...
            if (!isEditing && payload.building_name) {
                const selected = (data?.buildings || []).find((b) => String(b.id) === String(payload.building_name));
                if (selected) payload.building_name = selected.name;
            }

            if (isEditing) await axios.put(`${resource.endpoint}/${editingId}`, payload);
            else await axios.post(resource.endpoint, payload);
    ...
    const availableLayerBuildings = useMemo(() => {
        ...
        return (data?.buildings || []).filter((b) => !occupiedNames.has(b.name) && /^B-(0[4-9]|1[01])$/.test(b.name));
    }, [data]);
    ...
    const buildingFlockMap = useMemo(() => {
        ...
        (flockData?.items || []).forEach((item) => {
            if (item.status === 'active' && item.building_name) map[item.building_name] = item;
        });
        return map;
    }, [flockData]);

    const selectedBuildingFlock = dailyReportForm.building_id
        ? buildingFlockMap[(flockData?.buildings || []).find((b) => String(b.id) === String(dailyReportForm.building_id))?.name || ''] || null
        : null;
    ...
    const [transferFormState, setTransferFormState] = [transferForm, setTransferForm];
                                ...
                                return (
                                    <React.Fragment key={rowKey}>
                                        <tr className={[isMature && 'row-mature', dueforCull && 'row-cull', resource.expandable && 'expandable-row', isExpanded && 'expanded'].filter(Boolean).join(' ')}>
                                            ...
                                            <td>
                                                <RowActionButtons onView={() => setViewItem(item)} onEdit={() => openEdit(item)} />
                                            </td>
                                        </tr>
            ...
            {activeTab === 'chicken' && (
                <div className="grid-2 stock-widgets">
                    <PanelCard title="Chicken Distribution by Type" subtitle="Birds by flock type" icon="bi-pie-chart">
                        <SegmentDonut segments={buildFlockSegments(data?.distribution)} total={data?.summary?.totalPoultry} centerLabel="Birds" />
                    </PanelCard>
